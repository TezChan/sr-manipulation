...
from handle_demo_step_actions.msg import PlaceObjectActionFeedback  


class PlaceObjectControl(SequenceStep):
    """Class to control the object placing step"""
    def __init__(self, sequence, config):
        super(PlaceObjectControl, self).__init__(sequence, config)
        ...
        ui_file = os.path.join(self.ui_dir, 'PlaceObjectInfoArea.ui')
    ...
    def feedback_subscription(self):
        self.feedback_listener = rospy.Subscriber('place_object/feedback', PlaceObjectActionFeedback, self.feedback_callback)
